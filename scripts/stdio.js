...
//	var strSource = "../" + id + ".xml";
	var strSource = "../" + "pages" + "/"  + id + ".html";

	if (id.length==10) {
		strSource = "../" + id + "/" + id + ".htm";
		parent.MAIN.document.location.href = strSource;
	} else {
//Start of mine crutch
		strSource = "../" + "pages" + "/" + id + ".html";
		parent.MAIN.document.location.href = strSource;
//End of mine crutch --> Start of original code
...
// 		}
	...
	window.open(file, "xref_window", "menubar=yes,toolbar=no,locationbar=no,statusbar=no,scrollbars=yes,width=820,height=640,resizable=yes");
...
//Mine work
    var image = new Image();
    image.src = "../image/TOOL/" + "TT" + tnum + ".jpg";
    var width = image.width;
    var height = image.height;
    window.open(image.src,"Image","width=" + width + ",height=" + height);
...
//Start mine code 2
//src = '<html>';
//src += '<head><title>Tool Window</title><meta http-equiv="Content-Type" content="text/html; charset=utf-8"/></head>';
//src += '<body>';
//	src += '<img src="' + path + '" width="100%">';
//src += '<img src="' + tnum + '" width='"100%">';
//src += '</body>';
//src += '</html>';
//		window.open(src, "xref_window", "menubar=yes,toolbar=no,locationbar=no,statusbar=no,scrollbars=yes,width=440,height=400,resizable=yes");
//End mine code 2

//Seva
//    file = "../image/TOOL/" + "TT" + tnum + ".jpg";
//    window.open(file, "xref_window", "menubar=yes,toolbar=no,locationbar=no,statusbar=no,scrollbars=yes,width=440,height=400,resizable=yes");
}
...
//function showPrexns(sieid) {
function showPrexns(sieid) {

	var prexns_configid = "../" + "prexns" + "/" + sieid.substr(8,3) + ".html";

	window.open(prexns_configid, "prexns_window", "menubar=yes,toolbar=no,locationbar=no,statusbar=no,scrollbars=yes,width=560,height=600,resizable=yes");
//	if (parent.XREFMAIN || parent.PREXMAIN) {
//		var motherwindow = parent.opener;
//	}
//	else {
...
//		var htmltxt = loader.transform("../xml/webdocstructure.xml", "../config8_am/prexns.xsl", {"UI_lang": UI_lang, "targetId": prexns_configid, "srcWin": srcWin});
//		var htmltxt = loader.transform("../xml/webdocstructure.xml", "../config8_am/prexns.xsl", {"targetId": prexns_configid, "srcWin": srcWin});
...
function showPrexnSie(id) { 

//	var strSource = "../" + id + ".xml";
	strSource = "../" + "pages" + "/" + id + ".html";
	window.open(strSource, "PreXMainWindow","menubar=yes,toolbar=no,locationbar=no,statusbar=no,scrollbars=yes,width=820,height=640,resizable=yes");	
...
//		var UI_lang = opener.parent.CTL.document.ctl.lang.value;
//		var htmltxt = loader.transform(page, "../config8_am/suzuki_all.xsl", {"UI_lang": UI_lang, "ManualLinkage": loader.enableManualLinkage});
//		var htmltxt = loader.transform(page, "../config8_am/suzuki_all.xsl", {"ManualLinkage": loader.enableManualLinkage});
...
//	var src = genSubWindowContents("PreXns Main", "1", "CTL","", "PREXMAIN", '"' + strSource + '"');
//	var finally_func = function(win) {
//		win.focus();
