...
<html>
<title>Specification Search</title>
...
<script type="text/javascript" src="../scripts/ui.js" defer="defer" charset="utf-8"></script>
	...
	<xsl:value-of select="$newline"/>
<body onload="initSearchSpec()">
	<xsl:value-of select="$newline"/>
<table border="0" width="100%" cellpadding="0" cellspacing="0">
	<tr>
		<td align="center"><div class="listhead">
		<xsl:call-template name="getSpecSearchTitle">
			<xsl:with-param name="lang" select="$lang"/>
		</xsl:call-template>
		<xsl:text>&#160;</xsl:text>
		<img class="title_icon" onclick="setMode('navi_spec');" src="../icon/bookmark.gif" border="0"/>
		</div></td>
	</tr>
</table>
...
<tr>
<td align="left">
	<form class="searchconsole" name="searchspec" onSubmit="runSearch_Spec(); return false;">
	<table border="0" cellpadding="2" cellspacing="0">
	<tr valign="middle"><td align="left" colspan="2" style="padding-left:10px;">
		<xsl:call-template name="getUI_strSearchSpec">
			<xsl:with-param name="lang" select="$lang"/>
		</xsl:call-template>
	</td></tr>
...
<xsl:variable name="btnUI_Search">
	...
	<input type="button" name="search" value=" {$btnUI_Search} " onClick="runSearch_Spec(this.Form);"/>
	</td></tr>
	</table>
	</form>
	</td>
