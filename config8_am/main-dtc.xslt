...
<title><xsl:value-of select="//servinfo/@servinfosgmlid"/></title>
	...
	<xsl:choose>
		...
		<xsl:otherwise>
			<link rel="stylesheet" href="../ccss/speclists_e.css" type="text/css"  charset="utf-8"/>
		</xsl:otherwise>
	</xsl:choose>
	<xsl:value-of select="$newline"/>
...
<xsl:variable name="showThis">
	<xsl:text>0</xsl:text>
	<xsl:if test="$targetID='all'">
		<xsl:text>1</xsl:text>
	</xsl:if>
	<xsl:if test="$sct=$tgtSct and $sct!='z'">
		<xsl:text>1</xsl:text>
	</xsl:if>
</xsl:variable>
	<xsl:value-of select="$newline"/>
...
<xsl:if test="$targetID='all' or $targetID=../@id">
	<xsl:value-of select="$newline"/>
	<xsl:element name="div">
