...
<table border="0" width="100%" cellpadding="0" cellspacing="0">
	<tr>
		...
		<xsl:call-template name="getSdmIndexTitle">
			<xsl:with-param name="lang" select="$lang"/>
		</xsl:call-template>
		<xsl:text>&#160;</xsl:text>
		<img class="title_icon" onclick="setMode('search_sdm');" src="../icon/search_2.gif" border="0"/>
		</div></td>
	</tr>
</table>
...
<xsl:choose>
	<xsl:when test="$sct!=$prevsct and $sct!='az'">
		<xsl:element name="a">
			<xsl:attribute name="href">javascript: loadSdmList('<xsl:value-of select="$sct"/>')</xsl:attribute>
